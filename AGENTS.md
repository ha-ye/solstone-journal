...
| Local machine secrets (`~/Library/Application Support/Solstone/secrets/**`) | `solstone/think/importers/local_secrets.py` — machine-local OAuth tokens, API keys, service tokens, and migrated credential verifiers outside journal content, import bundles, indexes, and replicated backups. Secret files are keyed by active journal fingerprint except explicitly documented non-journal-scoped credentials. |
