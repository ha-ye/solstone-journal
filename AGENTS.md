...
| Operational-log/cache pruning — deletion only, across the dated allowlist (`chronicle/<day>/health/*.{log,jsonl}`, top-level `talents/<talent>/<epoch_ms>.jsonl` run logs + `talents/<YYYYMMDD>.jsonl` day indexes, `tokens/`, `awareness/`, `config/actions/`, `facets/*/logs/`, `apps/observer/observers/*/hist/`, `.cache/cogitate-history/`) | `solstone/think/log_retention.py` — prunes derived/operational artifacts by retention age; deletion only, not domain-state mutation. Routes its own deletes; does not write through each domain's owner. |
