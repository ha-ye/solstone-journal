...
3. Use `sol call activities list --source anticipated --day YYYYMMDD` across the last 7 days to understand scheduled load and major time commitments.
4. Use `journal identity pulse` for the current state narrative.
5. Use `sol call journal news FACET --day YYYYMMDD` for any facet that needs a richer summary.
