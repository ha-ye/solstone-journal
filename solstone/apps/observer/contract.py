...
OPERATIONS: list[OperationSpec] = [
    ...
    OperationSpec(
        ...
        description=(
            ...
            "(file at its recorded path) or `missing` (not found). A local file "
            "is proven held by the journal only when its listing entry is "
            "`present` AND the entry `sha256` matches the local file; a `missing` "
            "status — or any local file the listing does not positively account "
            "for — is needs-upload, and the client must never delete a local copy "
            "on that basis. Confirm-before-delete matches every upload-eligible "
