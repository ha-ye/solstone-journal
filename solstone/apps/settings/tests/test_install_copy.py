...
def test_new_local_install_copy_is_exported_and_populated() -> None:
    for name in NEW_LOCAL_COPY:
...
def test_new_local_install_copy_avoids_banned_owner_terms() -> None:
    combined = "\n".join(getattr(install_copy, name) for name in NEW_LOCAL_COPY)
