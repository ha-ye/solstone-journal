...
def _get_private_link_status() -> dict[str, Any]:
    return get_client().request("GET", "/app/network/api/private-link")
...
def private_link_setup(
    ...
    response = _post_private_link("/app/network/private-link/enable")
...
def private_link_disable() -> None:
    ...
    try:
        response = _post_private_link("/app/network/private-link/disable")
    except ConveyClientError as err:
...
def pair(
    ...
    try:
        resp = client.request("POST", "/app/network/pair-start", json=payload)
    except ConveyClientError as err:
    ...
    before = {
        d["fingerprint"]
        for d in client.request("GET", "/app/network/api/devices")["devices"]
    }
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        time.sleep(1.0)
        devices = client.request("GET", "/app/network/api/devices")["devices"]
...
def list_devices() -> None:
    ...
    devices = get_client().request("GET", "/app/network/api/devices")["devices"]
    if not devices:
...
def unpair(
    ...
    try:
        get_client().request("POST", "/app/network/unpair", json=payload)
    except ConveyClientError as err:
...
def status() -> None:
    ...
    client = get_client()
    state = client.request("GET", "/app/network/api/status")
    private_link = client.request("GET", "/app/network/api/private-link")
    paired_count = len(client.request("GET", "/app/network/api/devices")["devices"])
    ...
    typer.echo("Listen-WS state: (query convey /app/network/api/status for live state)")
