...
def _update_scout_block(
    ...
    def apply(config: dict[str, Any]) -> JournalConfigMutation[dict[str, Any]]:
        ...
        return JournalConfigMutation(changed=current_raw != block, value=block)
...
def provision_scout_handoff(payload: dict[str, Any]) -> None:
    ...
    log.debug("provisioned scout service for account_id=%s", values["account_id"])
    ...
    def apply(config: dict[str, Any]) -> JournalConfigMutation[DisableOutcome]:
        ...
        if not isinstance(scout_block, dict):
            return JournalConfigMutation(
                changed=False,
                value=DisableOutcome(was_enabled=False, env_key_preserved=False),
            )
