...
def default_describe_jobs() -> int:
    """Return the default screen-describe process count for repair mode."""
...
def describe_per_proc_jobs(effective_procs: int) -> int:
    ...
    """
    if (
        not isinstance(effective_procs, int)
        or isinstance(effective_procs, bool)
        or effective_procs < 1
    ):
        raise ValueError("effective_procs must be an integer >= 1")
