...
def test_build_init_prompt_uses_agent_name(monkeypatch):
    ...
    assert "for Astra" in prompt


def test_build_refresh_prompt_uses_agent_name(monkeypatch):
    monkeypatch.setattr(brain, "get_config", lambda: {"agent": {"name": "Astra"}})
