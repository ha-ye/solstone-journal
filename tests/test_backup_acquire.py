...
def test_inspect_restic_ready_bounds_authoritative_version_probe(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _patch_linux_amd64(monkeypatch)
    ...
    (tmp_path / ".install-complete").write_text(
        json.dumps(
            {
                "schema_version": readiness.RESTIC_SCHEMA_VERSION,
                "tool": readiness.RESTIC_TOOL,
                "version": readiness.RESTIC_VERSION,
    ...
    class HangingProc:
        ...
        def communicate(self, *, timeout: float):
            assert timeout == 0.01
            raise readiness.subprocess.TimeoutExpired([str(binary), "version"], timeout)

        def wait(self, *, timeout: float) -> int:
            assert timeout == 0.5
            if self.alive:
                raise readiness.subprocess.TimeoutExpired(
                    [str(binary), "version"], timeout
                )
    ...
    monkeypatch.setattr(readiness.os, "killpg", fake_killpg)

    assert readiness.inspect_restic_ready(tmp_path, version_timeout=0.01) == (
        None,
        "restic_incompatible",
    )
    assert len(procs) == 2
    assert all(not proc.alive for proc in procs)
