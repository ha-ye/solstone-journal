...
def test_provider_card_helpers_are_coherent():
    ...
    script = (
        ...
        "assertCoherent(meta);\n"
        "meta = providerCardMeta({generate_ready: true, cogitate_ready: true, issues: []}, 'local-byo', null, null);\n"
        "assert(meta.badgeLabel === 'Endpoint ready' && meta.badgeTone === 'ok', 'BYO ready uses endpoint badge');\n"
        "assert(!meta.primaryLabel && !meta.primaryAction && !meta.primaryHref, 'BYO ready has no bundled install action');\n"
        "assertCoherent(meta);\n"
        "meta = providerCardMeta({generate_ready: false, cogitate_ready: false, issues: ['local_endpoint_unreachable']}, 'local-byo', null, null);\n"
        "assert(meta.badgeLabel === 'Endpoint not ready' && meta.badgeTone === 'warn', 'BYO unavailable uses endpoint warning badge');\n"
        "assert(meta.primaryAction === null && meta.issueText === 'local_endpoint_unreachable', 'BYO unavailable does not install bundled artifacts');\n"
        "assertCoherent(meta);\n"
