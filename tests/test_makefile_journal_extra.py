...
"""Guard: `make install` selects the CUDA journal group only on x86_64.

The Makefile derives JOURNAL_GROUP from JOURNAL_VARIANT, auto-detected
...
arch -> group mapping so the x86_64 gate can't silently regress (the build-
manifest analog of test_llama_server_pins_cover_expected_platforms).

It resolves JOURNAL_GROUP the way the Makefile actually would on a given host
...
def _resolve_journal_group(arch: str, *, nvidia: bool) -> str:
    """Return the Makefile's JOURNAL_GROUP as it would resolve on a host whose
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        helper = bindir / "print-journal-group.mk"
        helper.write_text(
            f"include {MAKEFILE}\n"
            "__journal_group__:\n"
            "\t@printf 'JOURNAL_GROUP=[%s]\\n' '$(JOURNAL_GROUP)'\n",
            encoding="utf-8",
        )
        ...
        result = subprocess.run(
            ["make", "--no-print-directory", "-f", str(helper), "__journal_group__"],
            cwd=REPO_ROOT,
            env=env,
            capture_output=True,
            text=True,
        ...
        match = re.search(r"^JOURNAL_GROUP=\[(.*)\]$", result.stdout, re.MULTILINE)
        assert match is not None, (
            f"could not read JOURNAL_GROUP from make output\n"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("aarch64", True, "journal-cpu"),
        ("aarch64", False, "journal-cpu"),
        ...
        ("arm64", True, "journal-cpu"),
        ("arm64", False, "journal-cpu"),
    ],
)
def test_journal_group_selects_cuda_only_on_x86_64(
    arch: str, nvidia: bool, expected: str
) -> None:
    assert _resolve_journal_group(arch, nvidia=nvidia) == expected
